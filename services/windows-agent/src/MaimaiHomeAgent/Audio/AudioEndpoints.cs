...
public static class AudioEndpoints
{
    public static IEndpointRouteBuilder MapAudioEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            EventPublisher events) =>
        {
            ...
            // through silently.
            if (body is null || body.Level is null) return ValidationError("level must be between 0 and 1");
        ...
        app.MapPost("/api/audio/mute", async (
            ...
            EventPublisher events) =>
        {
            if (body is null || body.Muted is null) return ValidationError("muted must be true or false");
    ...
    {
        return Results.Ok(new AudioStateDto(state.MasterVolume, state.Muted, state.DefaultDeviceId));
    }
    ...
    /// </summary>
    private static IResult BusyResult()
    {
        return new BusyResultImpl();
    }
...
/// <summary>
///     Body of <c>POST /api/audio/volume</c>. <c>Level</c> is nullable so
///     missing fields surface as 400 rather than defaulting to 0.
/// </summary>
public sealed record VolumeRequest([property: JsonPropertyName("level")] double? Level);

/// <summary>
///     Body of <c>POST /api/audio/mute</c>. <c>Muted</c> is nullable for
///     the same reason as <see cref="VolumeRequest.Level" />.
/// </summary>
public sealed record MuteRequest([property: JsonPropertyName("muted")] bool? Muted);