...
public sealed class CoreAudioService : IAudioService, IAudioDeviceCacheInvalidator, IDisposable
{
    ...
    private const NAudioDeviceState ListedDeviceStates =
        NAudioDeviceState.Active | NAudioDeviceState.Disabled | NAudioDeviceState.NotPresent | NAudioDeviceState.Unplugged;
    ...
    public Task<AudioState> GetStateAsync()
        => _dispatcher.InvokeAsync(() => Task.FromResult(WithAudio(() =>
        {
            using var enumerator = new MMDeviceEnumerator();
            using var device = GetDefaultPlaybackDeviceOrNull(enumerator);
            return new AudioState(
                MasterVolume: device is null ? 0d : Math.Clamp(device.AudioEndpointVolume.MasterVolumeLevelScalar, 0f, 1f),
                Muted: device?.AudioEndpointVolume.Mute ?? false,
                DefaultDeviceId: device is null ? null : AudioDeviceId.FromEndpointId(device.ID));
    ...
    public Task<IReadOnlyList<AudioDevice>> ListDevicesAsync()
    {
        ...
        return _dispatcher.InvokeAsync(() => Task.FromResult(WithAudio<IReadOnlyList<AudioDevice>>(() =>
        {
            using var enumerator = new MMDeviceEnumerator();
            ...
            foreach (var device in devices)
            {
                using (device)
                {
                    result.Add(new AudioDevice(
                        Id: AudioDeviceId.FromEndpointId(device.ID),
    ...
    public Task SetDefaultDeviceAsync(Guid deviceId)
    {
        return _dispatcher.InvokeAsync(() =>
        {
            ...
            {
                using var enumerator = new MMDeviceEnumerator();
                var target = FindActiveDeviceEndpointId(enumerator, deviceId) ?? throw new AudioDeviceNotFoundException(deviceId);
                ...
                using var defaultDevice = GetDefaultPlaybackDeviceOrNull(enumerator);
                if (defaultDevice is null || !string.Equals(defaultDevice.ID, target, StringComparison.OrdinalIgnoreCase))
                {
                    throw new AudioOperationException($"Windows did not promote audio device {deviceId} to the default playback device.");
                }
    ...
    public bool TryUpdateDefaultDeviceCache(string? endpointId, out IReadOnlyList<AudioDevice> devices)
    {
        ...
        lock (_deviceCacheGate)
        {
            if (_deviceCache is null || DateTimeOffset.UtcNow >= _deviceCacheExpiresAt)
            {
                return false;
            }
    ...
    private static MMDevice GetDefaultPlaybackDeviceOrThrow(MMDeviceEnumerator enumerator)
        => GetDefaultPlaybackDeviceOrNull(enumerator)
    ...
    private static string? FindActiveDeviceEndpointId(MMDeviceEnumerator enumerator, Guid deviceId)
    {
        ...
        foreach (var device in devices)
        {
            using (device)
            {
                if (AudioDeviceId.FromEndpointId(device.ID) == deviceId)
                {
                    return device.ID;
                }
            }
    ...
    private static DeviceState MapState(NAudioDeviceState state) => state switch
    {
...
internal static class AudioDeviceId
{
    public static Guid FromEndpointId(string endpointId)
    {
        ...
        if (start >= 0 && end > start && Guid.TryParse(endpointId.Substring(start + 1, end - start - 1), out var parsed))
        {
            return parsed;
        }
...
internal sealed class PolicyConfigClient : IDisposable
{
    ...
    public PolicyConfigClient()
    {
        ...
        _comObject = Activator.CreateInstance(type) ?? throw new InvalidOperationException("Could not create PolicyConfigClient COM object.");
...
internal interface IPolicyConfig
{
    ...
    [PreserveSig] int SetDefaultEndpoint([MarshalAs(UnmanagedType.LPWStr)] string endpointId, PolicyRole role);
