...
public sealed class AgentSettingsService : IAgentSettingsService
{
    ...
    public async Task<AgentSettingsSnapshot> GetAsync(CancellationToken ct = default)
    {
        ...
        return new AgentSettingsSnapshot(
            AdminPasswordConfigured: !string.IsNullOrWhiteSpace(_adminOptions.CurrentValue.Password),
            AutoStartEnabled: autoStartEnabled,
            Launcher: LauncherSettingsDto.FromOptions(_launcherOptions.CurrentValue),
            FileRoots: _fileRootService.ListRoots().Select(FileRootSettingsDto.FromFileRoot).ToList(),
            RemoteShutdown: RemoteShutdownSettingsDto.FromOptions(_remoteShutdownOptions.CurrentValue));
    }

    public async Task<SettingsUpdateResult> UpdateAsync(AgentSettingsUpdateRequest request, CancellationToken ct = default)
    {
        ...
        if (errors.Count > 0)
        {
            return SettingsUpdateResult.Failed(errors);
        }
        ...
        {
            _fileRootService.Reload(request.FileRoots.Select(root => root.ToFileRoot()));
        }

        if (_events is not null)
        {
            _events.PublishSettingsUpdated(new { updatedAt = DateTimeOffset.UtcNow });
        }
    ...
    private static List<SettingsValidationError> Validate(AgentSettingsUpdateRequest request)
    {
        ...
        if (request.RemoteShutdown is { Enabled: true } && string.IsNullOrWhiteSpace(request.RemoteShutdown.ControlToken))
        {
    ...
    private async Task PersistAsync(AgentSettingsUpdateRequest request, CancellationToken ct)
    {
        ...
        File.Move(tempPath, _userConfigPath, overwrite: true);
    }
