...
public class EventHub
{
    ...
    /// </summary>
    public IReadOnlyCollection<WebSocketSession> Snapshot()
    {
    ...
    public async Task AddAsync(WebSocket socket, CancellationToken ct)
    {
        ...
        _logger.LogInformation("WebSocket session {SessionId} connected. TotalSessions={Total}", session.Id,
            _sessions.Count);
    ...
    public async Task RemoveAsync(Guid id, CancellationToken ct)
    {
        ...
        {
            _logger.LogInformation("WebSocket session {SessionId} removed. RemainingSessions={Total}", id,
                _sessions.Count);
    ...
    public virtual async Task BroadcastAsync(EventEnvelope envelope, CancellationToken ct = default)
    {
        ...
        foreach (var session in sessions) sendTasks.Add(SendSafelyAsync(session, envelope, ct));
