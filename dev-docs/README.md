...
| 移动端完整管理能力 | [需求](features/2026-06-14-移动端完整管理能力/需求.md) | [实现规划](features/2026-06-14-移动端完整管理能力/实现.md) | 规划中 |
| Avalonia 桌面 UI 迁移 | 暂无 | [Avalonia 桌面 UI 迁移方案](features/archive/2026-06-12-Avalonia桌面UI迁移/设计-Avalonia桌面UI迁移方案.md)、[内容变更说明](features/archive/2026-06-12-Avalonia桌面UI迁移/内容变更说明.md) | 已实现（已归档） |
...
| 时间 | 内容 |
|---|---|
| 2026-06-12 22:28 | 新增启动器全局关闭快捷键 `stopKey`，默认 `F12`，关闭当前启动项后重新显示启动器；导航和确认键仍只在启动器窗口激活时生效。 |
| 2026-06-12 19:21 | 移除启动项标题配置：Avalonia 设置窗口不再显示启动项“标题”，服务端不再要求标题必填，启动器卡片统一展示“名称”。 |
| 2026-06-12 19:15 | 移除启动项独立按键配置：Avalonia 设置窗口不再显示启动项“按键”，服务端不再要求启动项按键必填或唯一，启动器保留全局导航键和确认键。 |
| 2026-06-12 19:09 | Avalonia 桌面 UI 迁移已实现并归档：Windows Agent 移除 WinForms 依赖，设置窗口和启动器迁移到 Avalonia，托盘消息循环改为原生 Win32，并补充内容变更说明。 |
| 2026-06-12 18:35 | 新增 Avalonia 桌面 UI 迁移方案，规划用 Avalonia 替换 WinForms 设置窗口和应用启动器。 |
| 2026-06-12 | 补充 Windows Agent wiki：管理员鉴权、`/api/settings`、`/api/launcher/*` 接口说明；`capabilities` 增加 `settingsManagement` 和 `launcher`。 |
| 2026-06-11 17:57 | 更新开机启动选择器与手机管理员面板实现规划，记录 Launcher 运行服务、全屏窗口 host、管理 API、启动/关闭命令和测试完成。 |
| 2026-06-11 17:50 | 更新开机启动选择器与手机管理员面板实现规划，记录托盘设置窗口最小可用版完成，设置窗口通过统一设置服务加载和保存 JSON 配置。 |
